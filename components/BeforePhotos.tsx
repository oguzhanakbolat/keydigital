...
const BeforePhotos: FC<BeforePhotosProps> = ({
  setImages,
  images,
  setStep,
  step,
    ...
    if (cameraRef.current) {
      try {
        ...
        if (photo) {
          ...
          const permanentUri = await saveImagePermanently(photo.uri, "before");
          setShowCamera(false);
  ...
  const openGallery = async () => {
    ...
    if (result.assets) {
      ...
      // Resmi kalıcı konuma kaydet
      const permanentUri = await saveImagePermanently(image.uri, "before");
      setImages([...images, permanentUri]);
      setShowCamera(false);
    }
  };
  ...
  return (
    ...
    >
      <Text style={styles.text}>
        İş yapımı sırasında fotoğraflandırma yapınız
      </Text>
      ...
      <View style={styles.listContainer}>
        ...
        <TouchableOpacity style={styles.imageContainer} onPress={takePicture}>
          <View style={styles.iconContainer}>
            <Icon name="photos" size={150} color={Colors.text} />
          </View>
          <Icon name="plus" size={32} color={Colors.text} />
          <Text style={styles.text}>Fotoğraf Ekle</Text>
        </TouchableOpacity>
      </View>
